...
def ask():
    ...
    detailed_description = f"Title: {most_relevant_book['title']}\n\n" + \
                           ...
                           f"This book explores themes such as {', '.join([theme.strip().lower() for theme in most_relevant_book['summary'].split() if theme.istitle()])} " + \
                           f"and provides deep insights into {most_relevant_book['description'].split('.')[0].strip().lower()}."

    response = {
        'title': most_relevant_book['title'],
        'description': most_relevant_book['description'],
        'summary': most_relevant_book['summary'],
